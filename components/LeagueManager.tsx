...
const LeagueManager: React.FC<LeagueManagerProps> = ({ teams, onTeamUpdate, latestSimulatedWeek, schedule, onApplyScores }) => {
  ...
  const handleAddBowler = () => {
    if (!selectedTeam) return;
    const newBowler: Bowler = { id: Date.now(), name: 'New Bowler', average: 150, totalPins: 0, gamesBowled: 0 };
  ...
  const generateRoster = async () => {
    ...
    try {
        ...
        const newBowlers: Bowler[] = data.bowlers.map((b: {name: string, average: number}, index: number) => ({
            id: Date.now() + index, name: b.name, average: b.average, totalPins: 0, gamesBowled: 0
        }));
  ...
  useEffect(() => {
    if (nextWeekSchedule) {
      ...
      nextWeekSchedule.matchups.forEach(m => { teamIdsInMatchups.add(m.team1Id); teamIdsInMatchups.add(m.team2Id); });
      
      const newScores: WeeklyScores = { week: nextWeek, teamScores: [] };

      teams.forEach(team => {
        if(teamIdsInMatchups.has(team.id)){
            const bowlerScores: GameScore[] = team.bowlers.map(b => ({ bowlerId: b.id, scores: [0, 0, 0] }));
  ...
  const fetchScores = async () => {
      ...
      try {
        const response = await fetch('/api/fetch-scores', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ week: nextWeek, scheduledMatchups: nextWeekSchedule.matchups, teams: teams }),
        });

        if(!response.ok) {
            const errorData = await response.json();
            throw new Error(errorData.error || 'Failed to fetch scores from API.');
        }
        
        const fetchedScores: WeeklyScores = await response.json();
        setScores(fetchedScores);

      } catch (error) {
          setScoreFetchError(error instanceof Error ? error.message : 'An unknown error occurred.');
      } finally {
          setIsLoading(false);
      }
  };
  ...
  const handleScoreChange = (teamId: number, bowlerId: number, gameIndex: number, scoreValue: string) => {
    ...
    setScores(prevScores => {
        ...
        const newTeamScores = prevScores.teamScores.map(ts => {
            if (ts.teamId === teamId) {
        ...
        return { ...prevScores, teamScores: newTeamScores };
    });
  };
  ...
  return (
    <div className="bg-brand-secondary p-4 md:p-6 rounded-lg shadow-2xl grid grid-cols-1 xl:grid-cols-2 gap-6">
      <div>
        <h2 className="text-2xl font-bold mb-4 text-brand-gold">Roster Management</h2>
        {/* Roster management UI... */}
         <div className="mb-4">
          <select value={selectedTeamId || ''} onChange={(e) => setSelectedTeamId(Number(e.target.value))} className="w-full p-2 rounded bg-brand-primary border border-brand-accent focus:outline-none focus:ring-2 focus:ring-brand-gold">
            ...
            <div className="space-y-2">
              {selectedTeam.bowlers.map(bowler => (
                <div key={bowler.id} className="flex items-center gap-2">
                  <input type="text" value={bowler.name} onChange={(e) => handleBowlerChange(bowler.id, 'name', e.target.value)} className="flex-grow p-1 rounded bg-brand-secondary border border-brand-accent text-sm" />
                  <input type="number" value={bowler.average} onChange={(e) => handleBowlerChange(bowler.id, 'average', Number(e.target.value))} className="w-20 p-1 rounded bg-brand-secondary border border-brand-accent text-sm" />
      ...
      <div>
        <h2 className="text-2xl font-bold mb-4 text-brand-gold">Score Entry for Week {nextWeek}</h2>
        {nextWeek > 34 ? (<p className="text-brand-light">Season is over.</p>) : !nextWeekSchedule || nextWeekSchedule.matchups.length === 0 ? (<p className="text-brand-light">Next week is a position round. Scores are not entered here.</p>) : (
          <div className="bg-brand-primary p-4 rounded-lg">
            <div className="mb-4">
                <button onClick={fetchScores} disabled={isLoading} className="w-full px-4 py-2 bg-blue-600 hover:bg-blue-500 rounded-md font-semibold transition-colors duration-200 disabled:bg-gray-500 disabled:cursor-wait">
                   {isLoading ? 'Fetching...' : `Fetch Week ${nextWeek} Scores from LaneTalk.com`}
                </button>
                 {scoreFetchError && <p className="text-red-500 text-sm mt-2">{scoreFetchError}</p>}
            </div>
                                    ...
                                    {team.bowlers.map(bowler => {
                                        ...
                                        return (
                                            <div key={bowler.id} className="grid grid-cols-4 gap-2 items-center">
                                                ...
                                                {[0, 1, 2].map(gameIndex => (
                                                    <input key={gameIndex} type="number" min="0" max="300" value={bowlerScore?.scores[gameIndex] || 0} onChange={(e) => handleScoreChange(team.id, bowler.id, gameIndex, e.target.value)} className="w-full p-1 rounded bg-brand-secondary border border-brand-accent text-center" />
                                                ))}
                                            </div>
                                        )
                                    })}
